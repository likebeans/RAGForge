...
class Settings(BaseSettings):
    ...
    # Yaoyan 数据库连接（用于项目数据入库）
    yaoyan_database_url: str = "postgresql+asyncpg://kb:kb@localhost:5435/yaoyan"
    ...
    qdrant_collection_prefix: str = (
        "kb_"  # Collection 名称前缀，用于 collection 隔离模式
    )
    qdrant_shared_collection: str = (
        "kb_shared"  # 共享 Collection 名称，用于 partition 隔离模式
    )
    ...
    chunk_enrichment_max_tokens: int = (
        800  # 增强文本最大 token 数（qwen3 thinking 需要更多）
    )
    chunk_enrichment_context_chunks: int = 1  # 上下文 chunk 数量（前后各 N 个）
    chunk_enrichment_model: str | None = (
        None  # 增强使用的模型，None 使用默认 openai_model
    )
