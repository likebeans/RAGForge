...
async def test_extract_fields_from_pdf_returns_standardized_payload(monkeypatch):
    ...
    assert len(result["raw_response"]["project_list"]) > 0
    ...
    assert len(fake_chat.calls) == 4
    assert fake_chat.calls[0]["model"] == "qwen-doc-turbo"
    assert "简要介绍" in fake_chat.calls[1]["messages"][-1]["content"]
    assert "ND-003" in fake_chat.calls[2]["messages"][-1]["content"]
    assert "ND-004" in fake_chat.calls[3]["messages"][-1]["content"]
